...
var problemContainer = $('.problem-container');
var xMark = $('.x-mark');
var userData, levelData;

$.getJSON('static/users.json', function(data) {
	userData = data;
...
function updateTeamLevel(teamLevel) {
	console.log('level:',teamLevel);

	// var i = 0
	$('.problem-link').each(function(i, obj) {
		if (i < teamLevel) {
			$(obj).addClass('active')
			console.log(i)
		} 
...
$('.shift-camera-button').click(function () {
	...
	if (teamName in userData && userData[teamName] == $('#team-password').val()) {
		teamLevel = levelData[teamName];
		...
		TweenLite.to(camera.rotation, 3, { x: Math.PI / 2, ease: Power3.easeInOut }),
		TweenLite.to(camera.position, 2.5, { z: 20, ease: Power3.easeInOut }),
		TweenLite.to(camera.position, 3, { y: 0, ease: Power3.easeInOut }),
		TweenLite.to(plane.scale, 3, { x: 2, ease: Power3.easeInOut })]);

		updateTeamLevel(teamLevel);
		...
		console.log($('#team-password').val())
		alert('Wrong team credentials.')
	}
...
$('.problem-link').click(function () {
	...
	$('#problem-id').text(problemId)

	skyContainer.fadeOut()

	introTimeline.add([
	TweenLite.fromTo(skyContainer, 0.5, { opacity: 1 }, { opacity: 0, ease: Power3.easeIn }),
	TweenLite.to(camera.rotation, 3, { x: Math.PI / 2, ease: Power3.easeInOut }),
	TweenLite.to(camera.position, 2.5, { z: 20, ease: Power3.easeInOut }),
	TweenLite.to(camera.position, 3, { y: 120, ease: Power3.easeInOut }),
	TweenLite.to(plane.scale, 3, { x: 2, ease: Power3.easeInOut })]);
	...
	introTimeline.add([
	TweenLite.to(xMark, 2, { opacity: 1, ease: Power3.easeInOut }),
	TweenLite.to(problemContainer, 2, { opacity: 1, ease: Power3.easeInOut })]);
...
$('.x-mark').click(function () {
	...
	// TweenLite.to(problemContainer, 0.5, { opacity: 0, ease: Power3.easeInOut }),
	...
	outroTimeline.add([
	TweenLite.fromTo(problemContainer, 0.5, { opacity: 1 }, { opacity: 0, ease: Power3.easeIn }),
	TweenLite.to(camera.rotation, 3, { x: Math.PI / 2, ease: Power3.easeInOut }),
	TweenLite.to(camera.position, 2.5, { z: 20, ease: Power3.easeInOut }),
	TweenLite.to(camera.position, 3, { y: 0, ease: Power3.easeInOut }),
	TweenLite.to(plane.scale, 3, { x: 2, ease: Power3.easeInOut })]);
	...
	outroTimeline.add([
	// TweenLite.to(xMark, 2, { opacity: 0, ease: Power3.easeInOut }),
	TweenLite.to(skyContainer, 0.5, { opacity: 1, ease: Power3.easeIn })]);

	problemContainer.fadeOut()
